  ...
  useEffect(() => {
    const fetchPacientesAsignados = async () => {
      ...
      try {
        ...
        // 2. Procesar pacientes únicos
        const pacientesMap = new Map();

        await Promise.all(
          querySnapshot.docs.map(async (citaDoc) => {
            ...
            try {
              ...
              if (!pacienteDoc.exists() || pacienteDoc.data().rol !== "paciente") return;

              const pacienteData = pacienteDoc.data();
              pacientesMap.set(pacienteDoc.id, {
                id: pacienteDoc.id,
                nombre: pacienteData.nombre || "Sin nombre",
                apellido: pacienteData.apellido || "",
                email: pacienteData.email || "Sin email",
                telefono: pacienteData.telefono || "Sin teléfono",
                ultimaCita: citaData.fecha || "No especificada"
              });
            } catch (error) {
